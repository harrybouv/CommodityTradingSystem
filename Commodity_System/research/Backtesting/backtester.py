...
PROJECT_ROOT = THIS_FILE.parents[2]  # Commodity_System
...
def apply_rebalance(
    ...
    if mode == "daily":
        scheduled_weights = raw_weights.copy()
    else:
        if mode == "weekly":
        ...
        scheduled_weights = raw_weights.loc[rebalance_dates].copy()

    executed_rows = []
    previous_weights = None

    turnover_penalty = min(max(TURNOVER_PENALTY, 0.0), 1.0)
    rebalance_speed = 1.0 - turnover_penalty

    for date, desired_weights in scheduled_weights.iterrows():
        desired_weights = desired_weights.fillna(0.0).clip(lower=0.0)

        if previous_weights is None:
            executed_weights = desired_weights.copy()
        else:
            weight_change = desired_weights - previous_weights
            portfolio_distance = weight_change.abs().sum()

            if USE_REBALANCE_THRESHOLD:
                should_rebalance = portfolio_distance >= REBALANCE_THRESHOLD
            else:
                should_rebalance = True

            if should_rebalance:
                if USE_TURNOVER_PENALTY and turnover_penalty > 0:
                    executed_weights = (
                        previous_weights
                        + rebalance_speed * weight_change
                    )
                else:
                    executed_weights = desired_weights.copy()
            else:
        ...
        executed_weights = executed_weights.fillna(0.0).clip(lower=0.0)

        total_exposure = executed_weights.sum()

        if total_exposure > 1.0:
            executed_weights = executed_weights / total_exposure

        executed_rows.append(executed_weights)
        previous_weights = executed_weights.copy()

    executed = pd.DataFrame(
        executed_rows,
        index=scheduled_weights.index,
        columns=scheduled_weights.columns,
    )

    out = (
        executed
        .reindex(raw_weights.index)
        .ffill()
        .fillna(0.0)
    )
...
def apply_volatility_targeting(
    ...
    if len(common_dates) == 0 or len(common_tickers) == 0:
        diagnostics = pd.DataFrame(
            {
                "date": weights.index,
                "realised_portfolio_vol": np.nan,
                "vol_target_scale": 1.0,
                "pre_vol_target_exposure": weights.sum(axis=1).values,
                "post_vol_target_exposure": weights.sum(axis=1).values,
            }
        )

        return weights, diagnostics

    aligned_weights = weights.loc[common_dates, common_tickers].fillna(0.0)
    aligned_returns = returns.loc[common_dates, common_tickers].fillna(0.0)
    ...
    if not VOL_TARGETING_ENABLED:
        ...
        # Conservative: today's scale uses volatility already observed.
        # Because simulate_strategy shifts weights by one day, this avoids lookahead.
        scale = scale.shift(1).fillna(1.0)

    scaled_weights = weights.copy()

    scaled_weights.loc[common_dates, common_tickers] = (
        aligned_weights.multiply(scale, axis=0)
    )

    diagnostics = pd.DataFrame(
        {
            "date": common_dates,
            "realised_portfolio_vol": realised_vol.values,
            "vol_target_scale": scale.values,
            "pre_vol_target_exposure": aligned_weights.sum(axis=1).values,
            "post_vol_target_exposure": (
                aligned_weights.multiply(scale, axis=0)
            ).sum(axis=1).values,
        }
    )

    return scaled_weights.fillna(0.0), diagnostics
...
def main() -> None:
    ...
    model_weights, vol_targeting_diagnostics = apply_volatility_targeting(
        model_weights,
        returns,
    )
    ...
    model_weights.to_csv(OUTPUT_DIR / "executed_model_weights.csv")
    vol_targeting_diagnostics.to_csv(
        OUTPUT_DIR / "vol_targeting_diagnostics.csv",
        index=False,
    )
